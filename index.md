...
An R package that provides functions for undertaking epiphytotic
modelling and simulation studies in R.

## epiphytoolR aims

This package aims to be a collaborative project to provide a suite of
functions for epidemiologists to build models from. The functions should
be at their most basic and easy to adapt to the various epidemiological
models. We hope this becomes a dependency for many more models. We
welcome contributions additional branches to explore additional
functionality and features.

## What epiphytoolR is not

This does not provide full epidemiological models. See the following R
packages for flexible complete models:
...
- [Agriculture: Plant
  Pathology](https://cran.r-project.org/web/views/Agriculture.html#PlantPath)
...
`epiphytoolR` is not on CRAN but is aimed for a release sometime in
2025.

You can install a stable version from the `main` branch with the
following code
...
The `dev` branch will have newer bug fixes and more features, see
[NEWS.md](https://github.com/PaulMelloy/epiphytoolR/blob/dev/NEWS.md).
...
Create an [issue](https://github.com/PaulMelloy/epiphytoolR/issues) on
this repo.
...
Contributing is easy, you may notice the reuse of common functions in
plant epidemiological modelling. Translate these equations to R code and
add them to this package through a pull request.
...
Also use [roxygen](https://roxygen2.r-lib.org/) headers to describe the
function in the file. ‘Ctrl + Alt + Shift + R’ Inserts an Roxygen header
skeleton to edit.
...
Function naming should be concise and have the consistent use of
prefixes as follows. - `est_` estimate, usually used when the function
or equation is based on empirical evidence.  
- `get_` a wrapper for retrieving data or accessing an API, or
calculations form data. - `impute_` imputation functions.  
...
This package provides common base level functions that can be used in
modelling the spatial movement of splash and wind dispersed plant
disease inoculum.
...
Epidemiological models need clean consistent weather data, this package
contains functions to provide clean interpolated data for models.

Future versions will look towards an international standard for weather
data format.
